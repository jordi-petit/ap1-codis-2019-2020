/* mostra quantes maneres hi ha de posar n reines en un tauler n × n sense amenaçar-se */
...
void totes_reines(int n, vector<int>& q, vector<bool>& r, vector<bool>& d1, vector<bool>& d2, int& c, int i) {
    if (i == n) {
        ++c;
    } else {
        for (int j = 0; j < n; ++j) {
            if (es_pot_posar_reina(n, q, r, d1, d2, i, j)) {
                ...
                totes_reines(n, q, r, d1, d2, c, i + 1);
